// Q2NS SwapApp의 비동기 quantum 경계를 기존 P5-A federation에 연결한다.
...
class SwapTraceTag : public Tag
{
  public:
    ...
    void Serialize(TagBuffer b) const override { b.WriteU64(session); b.WriteU64(message); b.WriteU64(sequence); }
    void Deserialize(TagBuffer b) override { session=b.ReadU64(); message=b.ReadU64(); sequence=b.ReadU64(); }
...
Q2nsHeader ReadSwapResult(Ptr<const Packet> packet)
{
    ...
    uint8_t payload[10]; // Q2NS 기존 packed CtrlHeader: uint64 sid, uint8 m1,m2
    Require(packet->GetSize() >= sizeof(payload), "short Q2NS control packet");
    packet->CopyData(payload, sizeof(payload));
    uint64_t sid;
    std::memcpy(&sid, payload, sizeof(sid));
    Require(sid == tag.session && payload[8] <= 1 && payload[9] <= 1, "Q2NS control payload mismatch");
    Q2nsHeader h;
    h.kind=2; h.session=sid; h.message=tag.message; h.txSequence=tag.sequence;
...
void RunParticipant(Wire& wire)
{
    ...
    struct Session { int64_t commandAt; std::string root; uint64_t message; };
    ...
    auto receive = [&](uint32_t node, Ptr<Packet> packet, const Address&) {
        ...
        Require(repeater->RequestExternalBsm(h.session), "duplicate Q2NS BSM command");
    };
        ...
        else if (tag == "INJECT")
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                Simulator::ScheduleNow([&, result] {
                    Require(repeater->CompleteExternalBsm(result.id, result.m1, result.m2), "duplicate BSM completion");
                });
            }
            for (size_t i=0;i<correctionCount;++i)
            {
                ...
                Simulator::ScheduleNow([&, sid] {
                    Require(endpoint->CompleteExternalCorrection(sid), "duplicate correction completion");
                });
            }
...
int main(int argc, char** argv)
{
    ...
    cmd.AddValue("bridgeFd", "Inherited Unix socket from run_q2ns.py", bridgeFd);
    cmd.Parse(argc, argv);
    try
    {
        Require(bridgeFd >= 0, "launch with contrib/cosim/python/run_q2ns.py");
        Wire wire(bridgeFd);
        RunParticipant(wire);
        close(bridgeFd);
        return 0;
    ...
    {
        std::cerr << "cosim-q2ns: " << error.what() << '\n';
        Simulator::Destroy();
        return 1;
    }
}
