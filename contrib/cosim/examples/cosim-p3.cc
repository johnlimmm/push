// P3: 여러 session이 Controller → R → B의 실제 UDP 링크를 공유한다. 양자 상태는 Python 참가자만 소유한다.
...
class P3SwapHeader : public Header
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::cosim::P3SwapHeader")
                                .SetParent<Header>().AddConstructor<P3SwapHeader>();
        return tid;
    }
...
struct TraceBuffer
{
    ...
    uint64_t Add(const std::string& type, uint32_t node, const P3SwapHeader& h,
...
P3SwapHeader ReadMessage(Ptr<Packet> packet)
{
    P3SwapHeader h;
...
void TracePhy(TraceBuffer* trace, std::string type, uint32_t node, Ptr<const Packet> packet)
{
    ...
    Require(ipv4.GetProtocol() == 17, "P3 expects only UDP data packets");
...
void RunParticipant(Wire& wire)
{
    ...
    Parse(wire.Read(), tag, commandRate, commandDelay, resultRate, resultDelay,
          commandBytes, resultBytes, sessionCount);
    Require(tag == "CONFIG" && sessionCount > 0 && sessionCount <= 64 &&
            commandRate > 0 && resultRate > 0 && commandDelay >= 0 && resultDelay >= 0 &&
            commandBytes >= 60 && commandBytes <= 1400 && resultBytes >= 60 && resultBytes <= 1400,
            "invalid P3 CONFIG");
    struct Session { int64_t commandAt; std::string root; };
    ...
    for (size_t i = 0; i < sessionCount; ++i)
    {
        uint64_t sid;
        Session item;
        Parse(wire.Read(), tag, sid, item.commandAt, item.root);
        Require(tag == "SESSION" && sid > 0 && item.commandAt >= 0 && item.commandAt <= MAX_TIME &&
                sessions.emplace(sid, item).second, "invalid or duplicate SESSION");
    ...
    auto link = [&](uint32_t from, uint32_t to, uint64_t rate, int64_t delay,
        ...
        // 주소 할당의 AQM을 제거한다. 실제 장치 FIFO 대기는 Python 검증기가 별도로 검사한다.
    ...
    auto send = [&](uint32_t from, const Address& destination, P3SwapHeader h,
    ...
    // 같은 네트워크를 공유하며, 각 명령은 session-local request=1 / pair=1,2를 운반한다.
    for (auto sid : sessionOrder)
    {
        P3SwapHeader command;
        command.session = sid;
    ...
    agents[R]->SetResultCallback([&](int64_t at, const Result& result) {
        ...
        P3SwapHeader h;
        h.session = result.id; // 앱 콜백의 내부 token은 session ID다.
        h.kind = 2;
        h.request = 1; // wire의 request ID는 session-local로 유지한다.
        h.m1 = result.m1;
        h.m2 = result.m2;
        h.message = 0;
    ...
    while (true)
    {
        ...
        else if (tag == "INJECT")
        {
            ...
            Require(at == Simulator::Now().GetNanoSeconds() && count <= sessionCount, "invalid INJECT");
        ...
        else throw std::runtime_error("unknown P3 command");
    }
    Simulator::Destroy();
}
} // namespace
...
int main(int argc, char** argv)
{
    ...
    cmd.AddValue("bridgeFd", "Inherited Unix socket from run_p3.py", bridgeFd);
    cmd.Parse(argc, argv);
    try
    {
        Require(bridgeFd >= 0, "launch with contrib/cosim/python/run_p3.py");
        Wire wire(bridgeFd);
        RunParticipant(wire);
        close(bridgeFd);
        return 0;
