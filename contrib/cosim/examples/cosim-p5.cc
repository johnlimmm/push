// P5-A: 다중 session의 UDP/background FIFO를 P3의 공유 R processor에 연결한다.
...
class P5Header : public Header
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::cosim::P5Header")
                                .SetParent<Header>().AddConstructor<P5Header>();
        return tid;
    }
...
struct TraceBuffer
{
    ...
    uint64_t Add(const std::string& type, uint32_t node, const P5Header& h,
...
P5Header ReadMessage(Ptr<Packet> packet)
{
    P5Header h;
...
P5Header ReadWire(Ptr<const Packet> packet, bool hasPpp = true)
{
    ...
    Require(ipv4.GetProtocol() == 17, "P5 expects UDP only");
...
void RunParticipant(Wire& wire)
{
    ...
    Require(tag == "CONFIG" && commandRate > 0 && resultRate > 0 &&
            ...
            sessionCount > 0 && sessionCount <= 64, "invalid P5 CONFIG");
    ...
    auto send = [&](uint32_t from, const Address& destination, P5Header h,
    ...
    agents[R]->SetResultCallback([&](int64_t at, const Result& result) {
        ...
        P5Header h;
        h.session = result.id; // 앱 callback token은 session ID
        h.kind = 2;
        h.request = 1; // 외부 request ID는 session-local
        h.m1 = result.m1;
        h.m2 = result.m2;
        h.message = 0;
        auto local = trace.Add("BSM_RESULT_LOCAL", R, h, completionCauses.at(result.id));
        auto cause = EventIdString(local);
        h.message = sessions.at(h.session).message + 1;
    ...
    while (true)
    {
        ...
        else throw std::runtime_error("unknown P5 command");
    }
    Simulator::Destroy();
}
} // namespace
...
int main(int argc, char** argv)
{
    ...
    cmd.AddValue("bridgeFd", "Inherited Unix socket from run_p5.py", bridgeFd);
    cmd.Parse(argc, argv);
    try
    {
        Require(bridgeFd >= 0, "launch with contrib/cosim/python/run_p5.py");
        Wire wire(bridgeFd);
        RunParticipant(wire);
        close(bridgeFd);
        return 0;
