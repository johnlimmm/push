#!/usr/bin/env python3
"""P5-A: 실제 packet FIFO와 공유 R FIFO가 결합된 시간·상태 검증."""
...
def find_worker():
    found = [p for p in (NS3_DIR / 'build/contrib/cosim').rglob('*cosim-p5*')
             if p.is_file() and os.access(str(p), os.X_OK)]
    if len(found) != 1:
        raise RuntimeError('Build ./ns3 build cosim-p5 or specify --ns3-binary')
    return found[0]


class P5Participant(P1Participant):
...
class P5ValidationFailure(RuntimeError):
...
class P5FederationManager:
    ...
    def run(self, references=None):
        ...
        participant = P5Participant(self.binary, self.config, roots)
        ...
        try:
            ...
            participant.close()
        report = dict(schema_version=1, milestone='P5-A', model='joint-packet-and-R-FIFO-native-T1T2',
        ...
        try:
            ...
            raise P5ValidationFailure(report, error) from error
        return report
...
def main():
    ...
    failure = None
    try:
        report = P5FederationManager(json.loads(args.scenario.read_text()), args.ns3_binary).run()
    except P5ValidationFailure as error:
...
if __name__ == '__main__':
    try:
        ...
        print('cosim-p5: {}'.format(error), file=sys.stderr)
        sys.exit(1)
