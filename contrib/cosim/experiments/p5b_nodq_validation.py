...
from p5b_timing import no_dq_timing as analytic_timing
from quantum_scheduler import MAX_TIME_NS


def require(condition, message):
    if not condition:
        raise RuntimeError('P5-B No-Dq-R validation: ' + message)
...
def calculate_metrics(report):
    ...
    p = metrics['processor']
    p['model'] = 'unbounded-R-execution-capacity'
    p['execution_capacity'] = len(report['config']['sessions'])
    p['utilization'] /= p['execution_capacity']
...
def validate_packets(report):
    ...
    def require(ok, message):
        if not ok:
            raise RuntimeError('P5-B No-Dq-R packet validation: ' + message)
...
def validate_report(report):
    ...
    for sid, snap in sessions.items():
        ...
        require((bsm['request_id'], bsm['pair_a'], bsm['pair_b']) == (1, 1, 2), 'BSM resource IDs')
        require(bsm['processor_id'] == 2 + next(i for i,s in enumerate(config['sessions']) if str(s['session_id']) == sid), 'virtual execution lane')
        require(bsm['arrival_ns'] == bsm['start_ns'], 'No-Dq-R must start immediately')
    ...
    for sample, step in zip(samples, report['bridge_steps']):
        ...
        require(sample['waiting_sessions'] == waiting and sample['running_sessions'] == running, 'FIFO queue sample')
